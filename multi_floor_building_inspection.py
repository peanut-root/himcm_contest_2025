...
class MultiFloorBuildingInspection:
    ...
    def get_path_distance(self, start, end):
        ...
        Uses Manhattan distance as approximation for hallway-based movement."""
        # Manhattan distance (L1 norm) approximates movement through corridors
    ...
    def get_path_distance_3d(self, person_pos, person_floor, room_door, room_floor):
        ...
        if person_floor == room_floor:
            # Same floor: direct corridor distance
            return self.get_path_distance(person_pos, room_door)
        else:
            ...
            to_stair = self.get_path_distance(person_pos, stair_pos_from)
            vertical = self.stairwell.get_vertical_distance(person_floor, room_floor)
            from_stair = self.get_path_distance(stair_pos_to, room_door)
    ...
    def find_nearest_exit(self, person):
        ...
        to_stair = self.get_path_distance((person.x, person.y), stair_pos)

        # Go down to F1 exit
        vertical = self.stairwell.get_vertical_distance(person.floor, "F1")
        from_stair = self.get_path_distance(self.stairwell.get_position_on_floor("F1"), self.exit1)
    ...
    def greedy_assign(self, start1=(1, 4, "F1"), start2=(1, 4, "F1")):
        ...
        for person in [person1, person2]:
            ...
            if person.floor != exit_floor:
                # Go to stairwell
                stair_pos = self.stairwell.get_position_on_floor(person.floor)
                stair_dist = self.get_path_distance((person.x, person.y), stair_pos)
                person.add_waypoint(stair_pos[0], stair_pos[1], person.floor)
